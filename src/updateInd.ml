...
type env 'a =
  ...
  | Vcnt of ref int
  | Vbool of bool
  | Vevent_weights of list (option int)
  | Vnone ]
;
...
(* print *)
type ppp = Def.gen_person (string * string * int * Update.create * string) string
         ;
...
value print_foreach print_ast eval_expr =
  ...
  and print_foreach_pevent env p al list =
    ...
    let () = printf "foreach_pevent, %d events, %d weights\n"
                    (List.length list) (List.length weights)
    in
    let () = assert (List.length weights = List.length list) in
    loop True 1 (weights,list) where rec loop first cnt =
      fun
        [ ([w::ww],[n :: nn]) ->
          (* let () = assert (cnt <= List.length weights) in *)
          let env =
